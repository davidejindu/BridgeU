      ...
      if (response.ok) {
        ...
        const transformedConversations = data.map((conv: any) => {
          ...
          if (conv.members.length === 1) {
            ...
            groupName = conv.members
              .map((member: any) => `${member.firstName} ${member.lastName}`)
              .join(", ");
          } else {
            const firstThree = conv.members
              .slice(0, 3)
              .map((member: any) => `${member.firstName} ${member.lastName}`)
              .join(", ");
          ...
          let groupInitials = "";
          if (conv.members.length === 1) {
            groupInitials = `${conv.members[0].firstName.charAt(
              0
            )}${conv.members[0].lastName.charAt(0)}`;
          } else if (conv.members.length <= 3) {
            groupInitials = conv.members
              .map((member: any) => member.firstName.charAt(0))
              .join("");
          } else {
            groupInitials =
              conv.members
          ...
          return {
            ...
            timestamp: new Date(conv.lastMessageTime).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            }),
        ...
        if (navigationState?.selectedConversationId) {
          console.log("Navigation state found:", navigationState);
          console.log(
            "Looking for conversation ID:",
            navigationState.selectedConversationId
          );
          console.log("Available conversations:", transformedConversations);

          const conversationExists = transformedConversations.find(
            (conv: any) => conv.id === navigationState.selectedConversationId
          );

          console.log("Found conversation:", conversationExists);

          if (conversationExists) {
            console.log("Setting selected conversation and fetching messages");
            setSelectedConversation(navigationState.selectedConversationId);
            fetchMessages(navigationState.selectedConversationId);
          } else {
            console.log("Conversation not found in transformed conversations");
          }
          // Clear the navigation state to prevent re-selection on future renders
          window.history.replaceState({}, document.title);
        } else if (navigationState?.createConversationWith) {
          // If we need to create a conversation with a specific user, open the modal and pre-select them
          const userToMessage = navigationState.createConversationWith;
          setShowCreateConversation(true);
          setSelectedUsers([userToMessage]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Stay connected with your international student community
          </p>
        </div>
                      ...
                      <p className="text-sm text-gray-500">
                        ...
                        currentConversation.name.includes("+")
                          ...
                          : (() => {
                              ...
                              return (
                                currentConversation.members?.[0]?.university ||
                                "Student"
                              );
                            })()}
                      </p>
